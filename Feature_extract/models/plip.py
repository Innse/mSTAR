...
def process_image(i, image, processor, device):
    img = image[i].cpu()
...
def plip(device, gpu_num):
    ...
    def func(image):
        # inputs = processor(images=(image.cpu()), return_tensors="pt")
        ...
        with ThreadPoolExecutor() as executor:
            input = list(executor.map(process_image, range(image.shape[0]), [image]*image.shape[0], [processor]*image.shape[0], [device]*image.shape[0]))

        inputs = torch.cat(input).to(device)
