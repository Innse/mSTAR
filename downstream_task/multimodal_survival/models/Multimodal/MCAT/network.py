...
from .util import BilinearFusion, SNN_Block, Attn_Net_Gated
from .trans import TransformerEncoderLayer


class MCAT_Surv(nn.Module):
    def __init__(self, fusion="concat", path_size=1024, omic_sizes=[100, 200, 300, 400, 500, 600], n_classes=4, model_size_wsi: str = "small", model_size_omic: str = "small", dropout=0.25):
        super(MCAT_Surv, self).__init__()
        self.fusion = fusion
        self.omic_sizes = omic_sizes
        self.path_size = path_size
        self.n_classes = n_classes
        self.size_dict_WSI = {"small": [self.path_size, 256, 256], "big": [self.path_size, 512, 384]}
        ...
        path_encoder_layer = TransformerEncoderLayer(d_model=256, nhead=8, dim_feedforward=512, dropout=dropout, activation="relu")
        ...
        omic_encoder_layer = TransformerEncoderLayer(d_model=256, nhead=8, dim_feedforward=512, dropout=dropout, activation="relu")
    ...
    def forward(self, **kwargs):
        x_path = kwargs["x_path"].squeeze(0)
        x_omic = [kwargs["x_omic%d" % i].squeeze(0) for i in range(1, 7)]
...
def multi_head_attention_forward(
    ...
    if attn_mask is not None:
        assert (
            attn_mask.dtype == torch.float32 or attn_mask.dtype == torch.float64 or attn_mask.dtype == torch.float16 or attn_mask.dtype == torch.uint8 or attn_mask.dtype == torch.bool
