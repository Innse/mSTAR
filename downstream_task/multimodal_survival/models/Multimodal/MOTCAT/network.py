...
from .util import SNN_Block, Attn_Net_Gated, BilinearFusion
...
class OT_Attn_assem(nn.Module):
    ...
    def OT(self, weight1, weight2):
        ...
        if self.impl == "pot-sinkhorn-l2":
            ...
            flow = ot.sinkhorn(a=src_weight.detach(), b=dst_weight.detach(), M=cost_map_detach / cost_map_detach.max(), reg=self.ot_reg)
            ...
            flow = ot.unbalanced.sinkhorn_knopp_unbalanced(a=a, b=b, M=M_cost.double(), reg=self.ot_reg, reg_m=self.ot_tau)
