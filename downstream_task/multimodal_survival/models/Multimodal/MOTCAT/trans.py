...
from .trans_util import Linear, NonDynamicallyQuantizableLinear
from .trans_util import _mha_shape_check, _in_projection_packed, _in_projection, _get_activation_fn
...
def _scaled_dot_product_attention(
    ...
    if attn_mask is not None:
        attn.masked_fill_(attn_mask.to(torch.bool), float("-inf"))
    attn = F.softmax(attn, dim=-1)
    if attn_mask is not None:
        attn = attn.masked_fill(attn_mask.to(torch.bool), 0.0)
    if dropout_p > 0.0:
...
def multi_head_attention_forward(
    ...
    if key_padding_mask is not None:
        ...
        # Merge key padding mask and attention masks. # <------------------- CHANGED
        if attn_mask is None:
            attn_mask = key_padding_mask
        elif attn_mask.dtype == torch.bool:
            attn_mask = attn_mask.logical_or(key_padding_mask)
        else:
            attn_mask = attn_mask.masked_fill(key_padding_mask, 1.0)  # <------------------- CHANGED

    # convert mask to bool
    if attn_mask is not None and attn_mask.is_floating_point():  # <------------------- CHANGED
        attn_mask = attn_mask.to(torch.bool)  # <------------------- CHANGED
...
class MultiheadAttention(torch.nn.Module):
    ...
    def forward(
        self, query: Tensor, key: Tensor, value: Tensor, key_padding_mask: Optional[Tensor] = None, need_weights: bool = True, attn_mask: Optional[Tensor] = None, average_attn_weights: bool = True
    ) -> Tuple[Tensor, Optional[Tensor]]:
        r"""
        Args:
...
class TransformerEncoderLayer(Module):
    ...
    def forward(self, src: Tensor, src_mask: Optional[Tensor] = None, src_key_padding_mask: Optional[Tensor] = None, is_causal: bool = False) -> Tensor:
        ...
        if (
            src.dim() == 3
            and not self.norm_first
            and not self.training
            and self.self_attn.batch_first
            and self.self_attn._qkv_same_embed_dim
            and self.activation_relu_or_gelu
            and self.norm1.eps == self.norm2.eps
            and src_mask is None
            and not (src.is_nested and src_key_padding_mask is not None)
        ):
            tensor_args = (
                src,
            ...
            if (
                not torch.overrides.has_torch_function(tensor_args)
                and
                ...
                and (not torch.is_grad_enabled() or all([not x.requires_grad for x in tensor_args]))
            ):
