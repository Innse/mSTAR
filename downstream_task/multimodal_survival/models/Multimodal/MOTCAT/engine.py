...
class Engine(object):
    ...
    def train(self, data_loader, model, criterion, optimizer):
        ...
        for batch_idx, (data_ID, data_WSI, data_Omics, data_Event, data_Censorship, data_Label) in enumerate(dataloader):
            ...
            cnt = 0.0
            all_risk = 0.0
            index_chunk_list = split_chunk_list(data_WSI, 16384)
            for tindex in index_chunk_list:
                wsi_mb = torch.index_select(data_WSI, dim=1, index=torch.LongTensor(tindex).to(data_WSI.device)).cuda()
                hazards, S, _, _ = model(x_path=wsi_mb, x_omic1=data_Omics1, x_omic2=data_Omics2, x_omic3=data_Omics3, x_omic4=data_Omics4, x_omic5=data_Omics5, x_omic6=data_Omics6)
                loss_micro = criterion(hazards=hazards, S=S, Y=data_Label, c=data_Censorship)
                loss += loss_micro
...
def split_chunk_list(data, batch_size):
    numGroup = data.shape[1] // batch_size + 1
    feat_index = list(range(data.shape[1]))
