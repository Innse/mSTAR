...
class CMTA(nn.Module):
    def __init__(self, path_size=1024, omic_sizes=[100, 200, 300, 400, 500, 600], n_classes=4, fusion="concat", model_size="small"):
        ...
        ###
        self.size_dict = {
            "pathomics": {"small": [self.path_size, 256, 256], "large": [self.path_size, 512, 256]},
        ...
        if self.fusion == "concat":
            self.mm = nn.Sequential(*[nn.Linear(hidden[-1] * 2, hidden[-1]), nn.ReLU(), nn.Linear(hidden[-1], hidden[-1]), nn.ReLU()])
    ...
    def forward(self, **kwargs):
        ...
        genomics_features = torch.stack(genomics_features, dim=1)
        # pathomics embedding
        pathomics_features = self.pathomics_fc(x_path)
        # encoder
        # pathomics encoder
        cls_token_pathomics_encoder, patch_token_pathomics_encoder = self.pathomics_encoder(pathomics_features)  # cls token + patch tokens
        # genomics encoder
        cls_token_genomics_encoder, patch_token_genomics_encoder = self.genomics_encoder(genomics_features)  # cls token + patch tokens
        ...
        # decoder
        # pathomics decoder
        cls_token_pathomics_decoder, patch_token_pathomics_decoder = self.pathomics_decoder(pathomics_in_genomics.transpose(1, 0))  # cls token + patch tokens
        # genomics decoder
        cls_token_genomics_decoder, patch_token_genomics_decoder = self.genomics_decoder(genomics_in_pathomics.transpose(1, 0))  # cls token + patch tokens
